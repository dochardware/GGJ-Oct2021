  ...
  for (i = 0; i != MAX_PROJECTILES; i++) {
    ...
    if (projectiles[i].life_time != 0) {
      ...
      if (projectiles[i].moving) {
        if (TileAt2x2(tile_x, tile_y)) {
          projectiles[i].life_time = 0;
        }
        if (projectiles[i].dir.x == 1) {
          //Moving right
          if (TileAt2x2(tile_x + 1, tile_y) & COLLISION_LEFT) {
            projectiles[i].life_time = 0;
          }
        } else if (!projectiles[i].dir.x) {
          if (projectiles[i].dir.y == 1) {
            //Moving down
            if (TileAt2x2(tile_x, tile_y + 1) & COLLISION_TOP) {
              projectiles[i].life_time = 0;
            }
          } else {
            //Moving up
          ...
          //Moving left
          if (TileAt2x2(tile_x - 1, tile_y) & COLLISION_RIGHT) {
            projectiles[i].life_time = 0;
          }
        }
      }
